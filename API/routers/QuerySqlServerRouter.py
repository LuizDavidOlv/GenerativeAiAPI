...
router = APIRouter(prefix="/querysqlserver", tags=["Query SQL Server"])
...
async def query_sql_server(question: str, collection_name: str = "langchain"):
    try:
        ...
        politeDfsEmployeeMessageConfig = Messages.get_dfs_employee_messages(
            question, sql_query_result
        )
...
def queryDB(text):
    ...
    prompt_template = load_prompt(
        "C:\\Git\\Gen AI\\genai-api\\api\\db\\dbStructure.yaml"
    )
    ...
    agent_executor = create_sql_agent(
        llm=model, verbose=True, db=db, prompt=prompt_template
    )
...
async def database_lookup_internal(query: str, vdb_collection_name: str):
    ...
    operationTypesChain = await generate_sql_chain(
        "Get all OPERTN_NM types", 1, llm, pgVectorStore
    )
    operationTypesQueryResult = await get_query_result(
        operationTypesChain, sqlServerStore
    )

    operationTypeMessagesConfig = Messages.get_operation_type_messages(
        operationTypesQueryResult, query
    )
    ...
    countOperationMessagesConfig = Messages.get_count_operation_messages(
        operation_type, uncheckedSqlQueryChain
    )
    llmUpdatedQuery = llm(countOperationMessagesConfig).content
    revisedSqlQueryChain = await generate_sql_chain(
        llmUpdatedQuery, 5, llm, pgVectorStore
    )
    ...
    try:
        with aid_engine.connect() as conn:
            ...
            if dataframe.shape[0] > 100:
                logging.warning(
                    "Generated SQL query resulted in a dataset larger than 100 rows. This dataset has been truncated."
                )
...
async def generate_sql_chain(query, kwargs, llm, pg_vector_store):
    documents = pg_vector_store.similarity_search(query=query, k=kwargs)
    sql_generation_prompt = PromptTemplate(
        input_variables=["task", "context"], template=SQL_GENERATION_TEMPLATE
    )
    database_schema = "\n-------------------------------------------------------------------------------------------------------------------------\n".join(
        document.page_content for document in documents
    )
    sql_generation_chain = LLMChain(llm=llm, prompt=sql_generation_prompt, verbose=True)
    dts = datetime.datetime.now(pytz.timezone("US/Central")).strftime(
        "%Y-%m-%d %H:%M:%S"
    )
    chain_output = sql_generation_chain.run(
        {"task": query, "context": database_schema, "datetime": dts},
        metadata={"chain": "sql_generation_chain"},
    )
    return chain_output
