...
def word_length(text: str):
    ...
    llm_with_tools = llmConfig.bind(
        functions=[format_tool_to_openai_function(t) for t in toolsConfig]
    )
...
def current_events(question: str):
    ...
    agent = initialize_agent(
        tools, llmConfig, agent=AgentType.ZERO_SHOT_REACT_DESCRIPTION, verbose=True
    )
...
def current_events(
    question: str = "Who is Leo DiCaprio's girlfriend? What is her current age raised to the 0.43 power?",
):
    llmConfig = ChatOpenAI(model="gpt-3.5-turbo", temperature=0)
...
def prompt_with_memory(question: str):
    ...
    agent_executor = AgentExecutor(agent=agentConfig, tools=toolsConfig, verbose=True)
    preResult1 = agent_executor.invoke(
    ...
    preResult2 = agent_executor.invoke(
        {"input": sponsoredMessage, "chat_history": chat_history}
    )
...
def re_act_agent_with_chat_history(question: str):
    ...
    agent_executor = AgentExecutor(
        agent=agent, tools=toolsConfig, verbose=True, handle_parsing_errors=True
    )
...
def re_act_sql_access(question: str):
    ...
    messages = [
        ...
        HumanMessage(
            content=f"Provide only the answer to the question. Remove any aditional details:\n QUESTION: {question}"
        ),
    ]
